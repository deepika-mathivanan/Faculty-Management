...
// Create a default user with async/await
async function createDefaultUser() {
	try {
	  const existingUser = await User.findOne({ username: "Establishment" });
  
	  if (!existingUser) {
		const newUser = new User({ username: "Establishment" });
		await User.register(newUser, "admin123");  // Password: admin123
		console.log("Default admin user created: Establishment / admin123");
	  } else {
  ...
  // Call the function after ensuring the DB connection
  mongoose.connection.once("open", async () => {
	console.log("Database connection is open.");
	await createDefaultUser(); // Ensure user is created only after DB connection
  });
